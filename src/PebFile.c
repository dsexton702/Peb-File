...
#define MAX_ITEM_TEXT_LENGTH (35)
...
static int s_active_item_counts = 0;
...
static void todo_list_appends(void) {
...
static void todo_list_init(void) {
  ...
  if (dict_write_int8(iter, SPY_KEY_START, 0) != DICT_OK) {
    return;
  }
...
static void in_received_handler(DictionaryIterator *iter, void *context) {
  Tuple *append_tuple = dict_find(iter, SPY_KEY_START);
   Tuple *toggle_state_tuple = dict_find(iter, TODO_KEY_FETCH);
...
if (toggle_state_tuple) {
      ...
      todo_list_append(toggle_state_tuple->value->cstring);
